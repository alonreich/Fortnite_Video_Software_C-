    ...
    public class SpinningWheelSlider : Control
    {
        ...
        public override void Render(DrawingContext context)
        {
            ...
            var rimGrad = new LinearGradientBrush
            {
                ...
                {
                    new GradientStop(Color.Parse("#111a22"), 0.0),
                    new GradientStop(Color.Parse("#5c7c99"), 0.2),
                    new GradientStop(Color.Parse("#3e5871"), 0.5),
                    new GradientStop(Color.Parse("#1a2631"), 0.8),
                    new GradientStop(Color.Parse("#0d141a"), 1.0)
                }
            };
            
            var pen = new Pen(new SolidColorBrush(Color.Parse("#090c0f")), 1.5);
            context.DrawRectangle(rimGrad, pen, rect, 6, 6);
            
            var innerRect = rect.Deflate(3);
            
            var faceGrad = new LinearGradientBrush
            {
                StartPoint = new RelativePoint(0, 0, RelativeUnit.Relative),
                EndPoint = new RelativePoint(0, 1, RelativeUnit.Relative),
                GradientStops = new GradientStops
                {
                    new GradientStop(Color.Parse("#080c08"), 0.0),
                    new GradientStop(Color.Parse("#1e313d"), 0.25),
                    new GradientStop(Color.Parse("#3a6b6b"), 0.5),
                    new GradientStop(Color.Parse("#14222b"), 0.75),
                    new GradientStop(Color.Parse("#050805"), 1.0)
                }
            };
            
            var innerPenColor = Color.Parse("#000000");
            var innerPen = new Pen(new SolidColorBrush(Color.FromArgb(140, innerPenColor.R, innerPenColor.G, innerPenColor.B)), 1);
            ...
            for (int i = _range.min; i <= _range.max; i++)
            {
                ...
                string txt = i < _labels.Count ? _labels[i] : i.ToString();
            ...
            if (IsEnabled)
            {
                var selPen = new Pen(new SolidColorBrush(Color.Parse("#ff4d4d")), 2, lineCap: PenLineCap.Round);
                context.DrawLine(selPen, new Point(cx, 3), new Point(cx, 11));
                context.DrawLine(selPen, new Point(cx, h - 11), new Point(cx, h - 3));
            }
        }
    }
}
